   ...
   public static MultipleChoiceTest getMultipleChoiceTest(JSONObject testData) {
      try {
         ...
         for (int i = 0; i < array.length(); i++) {
            ...
            if(image==null) {
               questions.add(new MultipleChoiceTextQuiz(testData.getString("helpText"), testData.getString("question"), answers.getString(correctIndex), wrongAnswers));
            } else {
               questions.add(new MultipleChoiceImageQuiz(testData.getString("helpText"), testData.getString("question"), answers.getString(correctIndex), wrongAnswers, image));
            }
         }
   ...
   public static TrueFalseTest getTrueFalseTest(JSONObject testData) {
      try {
         ...
         for(int i=0; i<array.length(); i++) {
            ...
            String image = testObject.optString("image");
            if(image==null) {
               questions.add(new TrueFalseTextQuiz(testData.getString("helpText"), testData.getString("instructions"), testData.getBoolean("correctAnswer")));
            } else {
               questions.add(new TrueFalseImageQuiz(testData.getString("helpText"), testData.getString("instructions"), testData.getBoolean("correctAnswer"), image));
            }
         }
