...
\begin{figure}[H] 
\centering 
\includegraphics[scale=0.4]{img/UC_1.png} 
...
\scen{l'utente:
\begin{itemize}
\item \UC{UC1.1} può autenticarsi;
\item \UC{UC1.2} può deautenticarsi;
\item \UC{UC1.3} può giocare;
\item \UC{UC1.4}può ricevere informazioni sull'App;
\item \UC{UC1.5} può vedere i propri risultati salvati;
\item \UC{UC1.6} può visualizzare i percorsi vicini;
\item \UC{UC1.7} può modificare le credenziali di accesso.
\end{itemize}}\\\\ 
\att{Utente.}
...
\begin{figure}[H] 
\centering 
\includegraphics[scale=0.4]{img/UC_1_1.png} 
...
\scen{\begin{itemize}
\item \UC{UC1.1.1} l'utente esegue il login se è già registrato;
\item \UC{UC1.1.2} l'utente si registra e poi esegue il login (\UC{UC1.1.1}) se non è registrato.
\end{itemize}}\\\\ 
...
\begin{figure}[H] 
\centering 
\includegraphics[scale=0.4]{img/UC_1_1_1.png} 
...
\desc{l'utente inserisce la password con cui si è registrato in precedenza rispettando il requisito R0F1.2.6.4.}\\\\ 
\pre{la connessione ad internet è attiva e l'App fornisce la pagina di login.}\\\\ 
...
\begin{figure}[H] 
\centering 
\includegraphics[scale=0.4]{img/UC_1_1_2.png} 
...
la password non è sufficientemente sicura (\UC{UC1.1.4}).}\\\\ 
\pre{l'utente non è registrato.}\\\\ 
\post{l'utente è registrato nel database.}\\\\ 
\scen{\begin{itemize}
\item \UC{UC1.1.2.1} l'utente inserisce un indirizzo email valido;
\item \UC{UC1.1.2.2} l'utente inserisce un username univoco;
\item \UC{UC1.1.2.3} l'utente inserisce una password sufficientemente sicura.
\end{itemize}}\\\\ 
\scensec{Se i requisiti di validità non sono soddisfatti (si vedano \Req{R0F1.2.6} e figli) il sistema procede con l'estensione \UC{UC1.1.4}.}\\\\ 
\att{Utente non autenticato.}
...
\scen{\begin{itemize}
\item \UC{UC1.1.3.1} l'utente visualizza l'informazione che i dati inseriti sono errati;
\item \UC{UC1.1.3.2} l'utente può resettare la propria password.
\end{itemize}}\\\\ 
\att{Utente non autenticato.}
...
\casoduso{UC1.1.4}{Visualizza errori su dati registrazione} 
\desc{l'utente compie uno dei seguenti errori e viene notificato per ciò:
...
INCLUSIONE:
l'utente non autenticato ha l'opportunità di autenticarsi (\UC{UC1.1}).}\\\\ 
...
\begin{itemize}
\item \UC{UC1.3.1} l'utente gioca il percorso;
\item \UC{UC1.3.2} l'utente visualizza i dati del percorso appena concluso.
\end{itemize}
...
l'inclusione \UC{UC1.1}. A questo punto, gli utenti che ora sono autenticati possono procedere con:
\begin{itemize}
\item \UC{UC1.3.3} l'utente autenticato salva i risultati.
\end{itemize}}\\\\ 
\att{Utente.}
...
\begin{figure}[H] 
\centering 
\includegraphics[scale=0.4]{img/UC_1_3_1.png} 
...
\scen{\begin{itemize}
\item \UC{UC1.3.1.1} l'utente raggiunge la stazione;
\item \UC{UC1.3.1.2} l'utente svolge la prova della stazione in cui si trova;
\item \UC{UC1.3.1.3} l'utente viene invitato a cercare la prova seguente.
\end{itemize}

ESTENSIONE: se la prova conclusa è l'ultima l'utente visualizza il riepilogo \UC{UC1.3.2}.}\\\\ 
\att{Utente.}
...
\begin{figure}[H] 
\centering 
\includegraphics[scale=0.4]{img/UC_1_3_1_2.png} 
...
\scen{\begin{itemize}
\item \UC{UC1.3.1.2.1} l'utente visualizza una descrizione della prova;
\item \UC{UC1.3.1.2.2} l'utente gioca la prova;
\item \UC{UC1.3.1.2.3} l'utente ottiene un punteggio sulla prova.
\end{itemize}}\\\\ 
\att{Utente.}
...
\begin{figure}[H] 
\centering 
\includegraphics[scale=0.4]{img/UC_1_3_2.png} 
...
\begin{itemize}
 \item \UC{UC1.3.2.1} informazioni generali sulla durata (orario di inizio/fine, durata totale);
\item \UC{UC1.3.2.2} durata delle prove (somma della durata di ciascuna prova);
\item \UC{UC1.3.2.3} punteggio ottenuto ottenuto (somma dei punteggi ottenuti nelle varie prove);
\item \UC{UC1.3.2.4} punteggio medio ottenuto dagli utenti (media dei punteggi ottenuti su quel percorso);
\item \UC{UC1.3.2.5} visualizza la tappa in cui è stato ottenuto il maggior numero di punti;
\item \UC{UC1.3.2.6} visualizza la tappa in cui è stato ottenuto il minor numero di punti.
\end{itemize}}\\\\ 
\att{Utente.}
...
\scen{\begin{itemize}
\item \UC{UC1.4.1} l'utente visualizza una descrizione delle funzionalità dell'App;
\item \UC{UC1.4.2} l'utente visita il sito web dell'App;
\item \UC{UC1.4.3} l'utente contatta il supporto tecnico dell'App.
\end{itemize}}\\\\ 
\att{Utente.}
...
\begin{itemize}
\item \UC{UC1.5.3} l'utente viene informato della possibilità di salvare i percorsi effettuati se si autentica.
\end{itemize}
altrimenti, se l'utente non ha percorsi registrati:
\begin{itemize}
\item \UC{UC1.5.2} l'utente viene invitato a compiere un percorso per poi visualizzarne il dettaglio in questa pagina.
\end{itemize}
altrimenti:
\begin{itemize}
\item \UC{UC1.5.1} l'utente visualizza l'elenco dei percorsi svolti;
\item \UC{UC1.5.1.1} l'utente può visualizzare il dettaglio di un percorso salvato.
\end{itemize}}\\\\ 
\att{Utente.}
...
\scen{ 
\begin{itemize}\item Visualizza dettagli percorso svolto (\UC{UC1.5.1.1}). 
 \end{itemize}
}\\\\ 
\att{Utente autenticato.}
...
\scen{\begin{itemize}
\item \UC{1.6.1} l'utente visualizza gli edifici abilitati entro un certo raggio;
\item \UC{1.6.2} l'utente visualizza i 10 edifici più vicini.
\end{itemize}}\\\\ 
...
\begin{figure}[H] 
\centering 
\includegraphics[scale=0.4]{img/UC_1_6_1.png} 
...
\scen{attraverso \UC{UC1.6.1.1} l'utente inserisce un raggio entro cui effettuare la ricerca di edifici abilitati}\\\\ 
\att{Utente.}
...
\begin{figure}[H] 
\centering 
\includegraphics[scale=0.4]{img/UC_1_6_3.png} 
...
\begin{itemize}
\item \UC{UC1.6.3.1} attraverso le informazioni ``in-app'' sull'edificio;
\item \UC{UC1.6.3.2} accedendo al sito web relativo all'edificio;
\item \UC{UC1.6.3.3} contattando in maniera diretta l'edificio.
\end{itemize}}\\\\ 
...
\begin{figure}[H] 
\centering 
\includegraphics[scale=0.4]{img/UC_1_6_3_3.png} 
...
\begin{itemize}
\item \UC{UC1.6.3.3.1} via telefono;
\item \UC{UC1.6.3.3.2} via email;
\item \UC{UC1.6.3.3.3} via Facebook;
\item \UC{UC1.6.3.3.4} via Twitter.
i seguenti sono opzionali:
\item \UC{UC1.6.3.3.5} via \gl{Telegram};
\item \UC{UC1.6.3.3.6} via WhatsApp.
\end{itemize}}\\\\ 
\att{Utente.}
...
\casoduso{UC1.6.3.3.5}{Contatta edificio via \gl{Telegram}} 
\desc{l'utente può contattare l'edificio tramite \gl{Telegram}.}\\\\ 
\pre{l'edificio può essere contattato tramite \gl{Telegram}.}\\\\ 
\post{il dispositivo lancia \gl{Telegram} aprendo una nuova conversazione con l'account fornito dall'edificio.}\\\\ 
\att{Utente.}
...
\begin{figure}[H] 
\centering 
\includegraphics[scale=0.4]{img/UC_1_7.png} 
...
\begin{itemize}
\item \UC{UC1.7.1} l'utente autenticato modifica la mail con cui è registrato nel sito;
\item \UC{UC1.7.2} l'utente autenticato modifica la password con cui esegue il login;
\item \UC{UC1.7.3} l'utente modifica il proprio username.
\end{itemize}}\\\\ 
\att{Utente autenticato.}
...
\desc{l'utente è autenticato e desidera modificare la password di registrazione al sistema

ESTENSIONE:
se l'utente inserisce una password che non rispetta i requisiti \Req{R0F1.2.6.4}, all'utente non viene consentito di procedere e si procede invece con \UC{UC1.1.4}.}\\\\ 
