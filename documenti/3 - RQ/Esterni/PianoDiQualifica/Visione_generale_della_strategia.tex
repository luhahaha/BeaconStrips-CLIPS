...
\label{visioneGeneraleDellaStrategia}
	\subsection{Procedure di controllo di qualità di processo}
	\label{procedureDiControlloDiQualitaDiProcesso}
		...
		Inoltre si possono usare delle metriche per quantificare la qualità dei processi, tali metriche sono descritte nella sezione \hyperref[misureEMetriche]{Misure e metriche}.
	\subsection{Procedure di controllo di qualità di \gl{prodotto}}
	\label{procedureDiControlloDiQualitaDiProdotto}
		...
		\begin{itemize}
			...
			\item \textbf{verifica:} è un processo che determina se l'output di una fase è consistente, corretto e completo. Per tutta la durata del \gl{progetto} verranno svolte attività di verifica, i cui risultati sono e saranno riportati nell'\hyperref[resocontoDelleAttivitaDiVerifica]{Appendice}.
	...
	\label{pianificazioneStrategicaETemporale}
	...
	\label{risorse}
		...
		\label{metrichePerIProcessi}
			\paragraph{Modello \gl{CMM}}
			\label{modelloCMM}
			Il modello \gl{CMM} permette di valutare la maturità e la qualità dei processi. La valutazione viene effettuata tramite una scala di cinque livelli:
			\begin{enumerate}
			 \item \textbf{Initial}: È il livello più basso. Il SW prodotto è caratterizzato dall'essere costruito seguendo strategie occasionali e caotiche. Solo alcuni processi produttivi sono ben definiti ed i successi dipendono solo da sforzi individuali.
		...
		\label{metrichePerIDocumenti}
			\paragraph{Indice \gl{Gulpease}}
			\label{indiceGulpease}
		...
		\label{metrichePerIlCodice}
			\paragraph{Numero di parametri}
			\label{numeroDiParametri}
			...
			\label{numeroDiCampiDatiPerClasse}
			...
			\label{chiamateInnestateDiMetodi}
			...
			\label{riepilogo}
				\begin{tabella}{!{\VRule}l!{\VRule}l!{\VRule}l!{\VRule}}
					...
					\hyperref[metrichePerIProcessi]{Metriche per i processi} & & \\
					\hyperref[modelloCMM]{Modello \gl{CMM}} & Defined - Managed & Managed - Optimizing \\
					\hyperref[scheduleVariance]{Schedule Variance} & $\ge-10\%$ & $\ge-5\%$ \\
					\hyperref[budgetVariance]{Budget Variance} & $\ge-10\%$ & $\ge-5\%$ \\
					\hyperref[metrichePerIDocumenti]{Metriche per i documenti} & & \\
					\hyperref[indiceGulpease]{Indice \gl{Gulpease}} & 40 - 100 & 50 - 100 \\
					\hyperref[metrichePerIlCodice]{Metriche per il codice} & & \\
					\hyperref[numeroDiParametri]{Numero di parametri} & 0 - 8 & 0 - 5 \\
					\hyperref[complessitaCiclomatica]{Complessità ciclomatica} & 0 - 10 & 0 - 6 \\
					\hyperref[numeroDiCampiDatiPerClasse]{Numero di campi dati per classe} & 0 -16 & 0 - 10 \\
					\hyperref[livelloDAnnidamento]{Livello d'annidamento} & 0 - 6 & 0 - 4 \\
					\hyperref[gradoDiInstabilita]{Grado di instabilità} & 0 - 0.8 & 0.3 - 0.7 \\
					\hyperref[chiamateInnestateDiMetodi]{Chiamate innestate di metodi} & 0 - 8 & 0 - 5 \\
					\hyperref[coperturaDelCodice]{Copertura del codice} & 80\% - 100\% & 90\% - 100\% \\
					\hyperref[numeroDiLineePerMetodo]{Numero di linee per metodo} & $\le50$ & $\le40$ \\
