...
\item CLIPS::client: componente globale per il front end del prodotto che utilizza il design pattern \gl{MVP}. Si occupa di fornire un'interfaccia grafica dell'applicazione e di interagire con il lato server.
...
\caption{Schema package client}
 \end{figure}
\compDescrizione{componente globale per il front end del prodotto che utilizza il design pattern \gl{MVP}. Si occupa di fornire un'interfaccia grafica dell'applicazione e di interagire con il lato server.}
\compPadre{CLIPS}
\begin{compPackageContenuti}
...
\begin{classeRelazioni}
\classeRelazione{CLIPS::client::data}{Step}{}\end{classeRelazioni}
...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.5]{img/package/png/client--pathprogress.png}
\caption{Schema package client::pathprogress}
 \end{figure}
\compDescrizione{componente che gestisce i dati del percorso, l'individuazione e la lettura dei beacon e salva i risultati ottenuti nelle prove mentre si sta giocando}
\compPadre{client}
\begin{compClassi} \\
\begin{classe}{CLIPS::client::pathprogress::BeaconDiscover}
	...
	\begin{classeRelazioni}
		\classeRelazione{CLIPS::client::pathprogress}{BeaconDiscoverDelegate}{interfaccia utilizzata per notificare se il dispositivo è entrato o uscito dal raggio di un beacon}
	\end{classeRelazioni}
\end{classe}
\begin{classe}{CLIPS::client::pathprogress::BeaconDiscoverDelegate}
	...
	\begin{classeRelazioni}
		\classeRelazione{CLIPS::client::pathprogress}{BeaconDiscover}{classe che si interfaccia direttamente con i beacon fisici}
	\end{classeRelazioni}
\end{classe}
...
\begin{classeRelazioni}
\classeRelazione{CLIPS::client::pathprogress}{PathProgressControllerDelegate}{interfaccia utilizzata per notificare se si è in presenza del beacon del prossimo step o di uno del proximity}\end{classeRelazioni}
...
\classeUtilizzo{notifica se si è nel raggio del beacon del prossimo step o di uno di proximity}
\begin{classeRelazioni}
...
\end{classe}
\begin{classe}{CLIPS::client::pathprogress::RawBeacon}
...
\begin{componente}{CLIPS::client::viewcontroller::authentication}
	...
	\begin{compClassi} \\
		\begin{classe}{CLIPS::client::viewcontroller::authentication::AccountActivity}
			...
			\begin{classeRelazioni}
				\classeRelazione{CLIPS::client::viewcontroller::authentication}{AccountViewDelegate}{interfaccia per segnalare gli eventi della classe AccountView}\end{classeRelazioni}
		...
		\begin{classeRelazioni}
			\classeRelazione{CLIPS::client::viewcontroller::authentication}{AccountActivity}{classe controller che si occupa di interagire con AccountView}\classeRelazione{CLIPS::client::viewcontroller::authentication}{AccountViewDelegate}{interfaccia per segnalare gli eventi della classe AccountView}\end{classeRelazioni}
	\end{classe}\begin{classe}{CLIPS::client::viewcontroller::authentication::AccountViewDelegate}
	...
	\begin{classeRelazioni}
		\classeRelazione{CLIPS::client::viewcontroller::authentication}{AccountActivity}{classe controller che si occupa di interagire con AccountView}\classeRelazione{CLIPS::client::viewcontroller::authentication}{AccountView}{classe che rappresenta la schermata delle informazioni dell'utente}\end{classeRelazioni}
...
\begin{classeRelazioni}
	\classeRelazione{CLIPS::client::viewcontroller::authentication}{ForgotPasswordView}{classe che si occupa della visualizzazione della schermata per la richiesta di una nuova password}\classeRelazione{CLIPS::client::viewcontroller::authentication}{ForgotPasswordViewDelegate}{interfaccia per segnalare gli eventi della classe ForgotPasswordView}\end{classeRelazioni}
...
%\end{classeAttributi}
\begin{classeRelazioni}
	\classeRelazione{CLIPS::client::viewcontroller::authentication}{ForgotPasswordViewDelegate}{interfaccia per segnalare gli eventi della classe ForgotPasswordView}\end{classeRelazioni}
\end{classe}\begin{classe}{CLIPS::client::viewcontroller::authentication::ForgotPasswordViewDelegate}
...
\begin{classeRelazioni}
	\classeRelazione{CLIPS::client::viewcontroller::authentication}{ForgotPasswordActivity}{classe controller che si occupa di interagire con ForgotPasswordView}\classeRelazione{CLIPS::client::viewcontroller::authentication}{ForgotPasswordView}{classe che si occupa della visualizzazione della schermata per la richiesta di una nuova password}\end{classeRelazioni}
...
\begin{classeRelazioni}
	\classeRelazione{CLIPS::client::viewcontroller::authentication}{LoginView}{classe che si occupa della visualizzazione della schermata per il login}\classeRelazione{CLIPS::client::viewcontroller::authentication}{LoginViewDelegate}{interfaccia per segnalare gli eventi della classe LoginView}\end{classeRelazioni}
...
%\end{classeMetodi}
\begin{classeRelazioni}
	\classeRelazione{CLIPS::client::viewcontroller::authentication}{LoginViewDelegate}{interfaccia per segnalare gli eventi della classe LoginView}\end{classeRelazioni}
\end{classe}\begin{classe}{CLIPS::client::viewcontroller::authentication::LoginViewDelegate}
...
\begin{classeRelazioni}
	\classeRelazione{CLIPS::client::viewcontroller::authentication}{LoginActivity}{classe controller che si occupa di interagire con LoginView}\classeRelazione{CLIPS::client::viewcontroller::authentication}{LoginView}{classe che si occupa della visualizzazione della schermata per il login}\end{classeRelazioni}
...
\begin{classeRelazioni}
	\classeRelazione{CLIPS::client::viewcontroller::authentication}{RegistrationView}{classe che si occupa della visualizzazione della schermata per la registrazione}\classeRelazione{CLIPS::client::viewcontroller::authentication}{RegistrationViewDelegate}{interfaccia per segnalare gli eventi della classe RegistrationView}\end{classeRelazioni}
...
\begin{classeRelazioni}
	\classeRelazione{CLIPS::client::viewcontroller::authentication}{AccountViewDelegate}{interfaccia per segnalare gli eventi della classe AccountView}\end{classeRelazioni}
\end{classe}\begin{classe}{CLIPS::client::viewcontroller::authentication::RegistrationViewDelegate}
...
\begin{classeRelazioni}
	\classeRelazione{CLIPS::client::viewcontroller::authentication}{RegistrationActivity}{classe controller che si occupa di interagire con RegistrationView}\classeRelazione{CLIPS::client::viewcontroller::authentication}{RegistrationActivity}{classe controller che si occupa di interagire con RegistrationView}\classeRelazione{CLIPS::client::viewcontroller::authentication}{RegistrationView}{classe che si occupa della visualizzazione della schermata per la registrazione}\end{classeRelazioni}
...
\begin{classeRelazioni}
	\classeRelazione{CLIPS::client::viewcontroller::authentication}{UpdateUserInfoView}{classe che si occupa della visualizzazione della schermata per il cambio delle credenziali}\classeRelazione{CLIPS::client::viewcontroller::authentication}{UpdateUserInfoViewDelegate}{interfaccia per segnalare gli eventi della classe UpdateUserInfoViewDelegate}\end{classeRelazioni}
...
\begin{classeRelazioni}
	\classeRelazione{CLIPS::client::viewcontroller::authentication}{UpdateUserInfoViewDelegate}{interfaccia per segnalare gli eventi della classe UpdateUserInfoViewDelegate}\end{classeRelazioni}
\end{classe}\begin{classe}{CLIPS::client::viewcontroller::authentication::UpdateUserInfoViewDelegate}
...
\begin{classeRelazioni}
	\classeRelazione{CLIPS::client::viewcontroller::authentication}{UpdateUserInfoActivity}{classe controller che si occupa di interagire con UpdateUserInfoView}\classeRelazione{CLIPS::client::viewcontroller::authentication}{UpdateUserInfoActivity}{classe controller che si occupa di interagire con UpdateUserInfoView}\classeRelazione{CLIPS::client::viewcontroller::authentication}{UpdateUserInfoView}{classe che si occupa della visualizzazione della schermata per il cambio delle credenziali}\end{classeRelazioni}
\end{classe}\end{compClassi}
\end{componente}
\begin{componente}{CLIPS::client::viewcontroller::building}
\begin{figure}[h!]
\centering
\includegraphics[scale=0.45]{img/package/png/client--viewcontroller--building.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.5]{img/package/png/client--viewcontroller--utility.png}
...
\begin{classeRelazioni}
\classeRelazione{CLIPS::client::viewcontroller::authentication}{AccountViewDelegate}{interfaccia per segnalare gli eventi della classe AccountView}\classeRelazione{CLIPS::client::viewcontroller::utility}{AppInfoView}{classe che si occupa delle informazioni generali dell'app}\end{classeRelazioni}
...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.45]{img/package/png/server--data.png}
