...
\begin{classeMetodoArgomenti}
\classeMetodoArgomento{algorithmData}{LinearScoringAlgorithm}{algoritmo per il punteggio}
\end{classeMetodoArgomenti}
\end{classeMetodi}
\begin{classeRelazioni}
...
\end{classeMetodoArgomenti}
\classeMetodo{returnResponse}{}{void}{chiama il metodo onResponse del listener e ritorna sempre "true", è un valore che serve solo per mantenere la consistenza della struttura del listener perché già chiamare il metodo indica che l'operazione è riuscita}
\end{classeMetodi}
\begin{classeRelazioni}
...
\classeAttributo{maxNumber}{int}{il numero massimo di edifici richiesti o la distanza massimi entro cui devono essere presenti gli edifici restituiti, a seconda del valore di ''searchByDistance''}
\classeAttributo{searchByDistance}{boolean}{indica se la richiesta al server deve ritornare gli edifici pi� vicini secondo un numero massimo o una distanza massima entro cui essi sono presenti}
\end{classeAttributi}
\begin{classeRelazioni}
...
\classeAttributo{type}{string}{il tipo di dato controllato, i possibili valori attesi sono "email", "password" e "username"}
\end{classeAttributi}
...
\classeAttributo{cachePolicy}{CachePolicy}{indica il comportamento dell'applicazione per la memorizzazione dei dati richiesti: "NoCache" indica che i dati vengono richiesti al server e mai salvati in locale; "AlwaysReplaceLocal" significa che i dati vengono richiesti al server, se vengono ricevuti allora i dati del database locale corrispondenti vengono sostituiti da quelli appena ottenuti, altrimenti vengono usati i dati salvati in locale; "LocalElseRemote" indica che vengono usati i dati del database locale, se non ci sono vengono richiesti al server}
\classeAttributo{cx}{Context}{è una classe delle librerie base di Android che costituisce il cuore dell'applicazione, e quindi è necessaria per poter effettuare alcune operazioni, tra cui le richieste al server}
...
\end{classeMetodoArgomenti}
\classeMetodo{getBuildings}{cx, latitude, listener, longitude, maxNumber, searchByDistance}{void}{crea l'oggetto per ottenere la lista degli edifici da visualizzare nell'applicazione, prima cercandoli nel server e poi, in caso ci siano problemi, nel database locale}
\begin{classeMetodoArgomenti}
\classeMetodoArgomento{cx}{Context}{è una classe delle librerie base di Android che costituisce il cuore dell'applicazione, e quindi è necessaria per poter effettuare alcune operazioni, tra cui le richieste al server}
...
\classeMetodoArgomento{maxNumber}{int}{valore che indica il numero massimo di edifici restituiti dal server o la loro distanza massima, a seconda del valore di ''searchByDistance''}
\classeMetodoArgomento{searchByDistance}{boolean}{valore che indica se la chiamata deve restituire gli edifici in base ad un numero massimo o ad una distanza massima}
\end{classeMetodoArgomenti}
...
\end{classe}\begin{classe}{CLIPS::client::data::datamanager::DBHandlerFIXME}
...
\classeMetodo{isLogged}{}{boolean}{restituisce "true" se l'utente è loggato, ovvero se è salvato un token in locale, "false" altrimenti}
...
\begin{classeMetodi}
\classeMetodo{setBody}{latitude, longitude, maxNumber, searchByDistance}{JSONObject}{costruisce la sezione body dell'oggetto JSON, è un metodo di supporto}
\begin{classeMetodoArgomenti}
...
\classeMetodoArgomento{maxNumber}{int}{indica il numero massimo di edifici richiesto o la distanza massima entro cui essi sono situati, a seconda del valore di ''searchByDistance''}
\classeMetodoArgomento{searchByDistance}{boolean}{indica se la ricerca degli edifici pi� vicini deve essere effettuata in base ad un numero massimo o ad una distanza entro cui devono essere situati}
\end{classeMetodoArgomenti}
\end{classeMetodi}
\begin{classeRelazioni}
\classeRelazione{CLIPS::client::data::urlrequest}{AbstractUrlRequestListener}{interfaccia che definisce la struttura del listener astratto usato nel package urlrequest}\classeRelazione{CLIPS::client::data::urlrequest}{URLDataConstants}{classe che contiene i dati per effettuare le richieste al server che potrebbero variare nel tempo}\end{classeRelazioni}
...
\begin{classeMetodi}
\classeMetodo{setBody}{}{JSONObject}{Crea l'oggetto JSON da inserire nella chiamata per poter inviare l'email dell'account di cui bisogna cambiare la password con una generata automaticamente}
\end{classeMetodi}
\begin{classeRelazioni}
...
\classeMetodo{signalError}{}{void}{cambia il valore di buildingJSONError in ''true'' e segnala quindi all'URLRequest che c'è stato un errore in fase di costruzione di ''body'', viene invocato dalle classi derivate di URLRequest}
\end{classeMetodi}
\begin{classeRelazioni}
...
\end{classeAttributi}
\begin{classeMetodi}
\classeMetodo{onBeaconServiceConnect()}{}{void}{metodo chiamato una volta che il dispositivo dell'utente si è collegato ad un Beacon}
\end{classeMetodi}
...
\begin{classeMetodi}
\classeMetodo{onError}{error}{void}{Restituisce il messaggio di errore quando ne avviene uno durante la rilevazione delle coordinate dell'utente con il GPS}
\begin{classeMetodoArgomenti}
\classeMetodoArgomento{error}{ServerError}{L'errore restituito durante la rilevazione del GPS}
\end{classeMetodoArgomenti}
\classeMetodo{onResponse}{latitude, longitude}{void}{Restituisce le coordinate GPS dell'utente rilevate come risposta}
\begin{classeMetodoArgomenti}
\classeMetodoArgomento{latitude}{double}{La latitudine dell'utente rilevata dal GPS}
\classeMetodoArgomento{longitude}{double}{La longitudine dell'utente rilevata dal GPS}
\end{classeMetodoArgomenti}
\end{classeMetodi}
\begin{classeRelazioni}
\classeRelazione{CLIPS::client::data::urlrequest}{ServerError}{classe che rappresenta l'errore che il client riceve quando la richiesta al server non ha successo}\end{classeRelazioni}
...
\end{classeMetodoArgomenti}
\end{classeMetodi}
\end{classe}\begin{classe}{CLIPS::client::pathprogress::PathProgressMaker}
\classeDescrizione{classe che funziona come interfaccia tra le classi per rilevare la posizione con il GPS e il resto dell'applicazione}
\classeUtilizzo{quando all'applicazione serve la posizione dell'utente invoca il relativo metodo della classe}
\begin{classeMetodi}
\classeMetodo{getCoordinates}{cx, listener}{void}{costruisce l'oggetto per rilevare le coordinate dell'utente con il GPS}
\begin{classeMetodoArgomenti}
\classeMetodoArgomento{cx}{Context}{è una classe delle librerie base di Android che costituisce il cuore dell'applicazione, e quindi è necessaria per poter effettuare alcune operazioni, tra cui le richieste al server}
\classeMetodoArgomento{listener}{GPSListener}{il listener che verrà  usato per informare l'applicazione se l'operazione prevista ha avuto successo o meno}
\end{classeMetodoArgomenti}
\end{classeMetodi}
\begin{classeRelazioni}
...
\begin{classeMetodoArgomenti}
\classeMetodoArgomento{buildings}{JSON}{array di oggetti JSON contenenti gli edifici cui si vogliono aggiungere le descrizioni dei percorsi: in particolare ciascun oggetto JSON deve contenere almeno la chiave \texttt{id} con l'id dell'edificio}
\end{classeMetodoArgomenti}
\end{classeMetodi}
\begin{classeRelazioni}
...
\begin{classeMetodi}
\classeMetodo{sendMail}{}{Promise}{invia la mail dall'indirizzo beaconstrips.swe@gmail.com}
\end{classeMetodi}
...
\begin{classeMetodi}
\classeMetodo{userData}{userID}{Promise}{metodo che a partire dall'userID restituisce i dati utente (email e username)}
\begin{classeMetodoArgomenti}
\classeMetodoArgomento{userID}{string}{id dell'utente}
\end{classeMetodoArgomenti}
\end{classeMetodi}
...
\begin{classeMetodi}
\classeMetodo{sendNewPassword}{email, password}{Promise}{invia una mail contenente la nuova password all'indirizzo email indicato}
\begin{classeMetodoArgomenti}
\classeMetodoArgomento{email}{string}{l'email dell'utente cui comunicare la nuova password}
\classeMetodoArgomento{password}{string}{la nuova password da comunicare all'utente}
\end{classeMetodoArgomenti}
\end{classeMetodi}
...
\begin{classeMetodi}
\classeMetodo{getEmail}{}{Promise}{ritorna una promise che si risolve con successo solamente se con la richiesta viene fornita una email associata a qualche utente registrato}
\end{classeMetodi}
...
\begin{classeMetodoArgomenti}
\classeMetodoArgomento{proof}{JSON}{la prova che si vuole completare inserendo dati derivati; proof deve essere un JSON contenente almeno le chiavi \texttt{testData} e \texttt{scoringAlgorithmData} con l'id del test da integrare e l'id dell'algoritmo per i punteggi}
\end{classeMetodoArgomenti}
...
\begin{classeMetodoArgomenti}
\classeMetodoArgomento{proximity}{JSON}{la Proximity che si vuole completare inserendo dati derivati; proximity � un JSON che deve contenere almeno la chiave \texttt{beaconID} con l'id del beacon che viene usato per la prossimit�}
\end{classeMetodoArgomenti}
...
\begin{classeMetodoArgomenti}
\classeMetodoArgomento{step}{JSON}{la tappa che si vuole completare inserendo dati derivati; in particolare step � un JSON object che deve contenere almeno le chiavi \texttt{stopbeaconID} con l'id del beacon della stazione, \texttt{proof} con un oggetto di tipo Proof e \texttt{proximities} con un array di oggetti Proximity}
\end{classeMetodoArgomenti}
...
\begin{classeMetodoArgomenti}
\classeMetodoArgomento{proof}{JSON}{la prova di cui si vuole ottenere l'algoritmo; � un oggetto JSON che rappresenta l'oggetto Proof}
\end{classeMetodoArgomenti}
...
\begin{classeMetodoArgomenti}
\classeMetodoArgomento{step}{JSON}{tappa di cui si vogliono recuperare i dati relativi alla prova, l'oggetto JSON deve contenere almeno la chiave \texttt{proofID} con l'id della Proof}
\end{classeMetodoArgomenti}
...
\begin{classeMetodoArgomenti}
\classeMetodoArgomento{step}{JSON}{tappa di cui si vogliono avere le proximities; step � un JSON che deve contenere almeno la chiave \texttt{id} con l'id della stazione }
\end{classeMetodoArgomenti}
...
\begin{classeMetodoArgomenti}
\classeMetodoArgomento{path}{JSON}{path di cui si vogliono avere le tappe, in particolare l'oggetto JSON path deve contenere la chiave \texttt{id} con l'id del percorso}
\end{classeMetodoArgomenti}
...
\begin{classeMetodoArgomenti}
\classeMetodoArgomento{proof}{JSON}{la prova di cui si vogliono ottenere i dati: in particolare il JSON deve contenere almeno le chiavi \texttt{testData} e \texttt{algorithmData} con l'id dei test e degli algoritmi}
\end{classeMetodoArgomenti}
\end{classeMetodi}
\begin{classeRelazioni}
\classeRelazione{CLIPS::server::urlrequesthandler}{DBHandler}{classe che si occupa di gestire il DB e di ritornare un oggetto configurato della libreria knex}\end{classeRelazioni}
...
\begin{classeMetodi}
\classeMetodo{getNewData}{}{Promise}{estrapola dalla richiesta i nuovi dati da salvare nel database}
\classeMetodo{validateData}{data}{Promise}{ritorna una promise che viene risolta con successo solamente quando tutti i dati in input sono validi}
\begin{classeMetodoArgomenti}
\classeMetodoArgomento{data}{JSON}{un JSON con almeno una tra: le chiave \texttt{username} con il nuovo username, la chiave \texttt{email} con la nuova email, la coppia di chiavi \texttt{oldPassword} e \texttt{newPassword} con la vecchia e la nuova password.}
\end{classeMetodoArgomenti}
\classeMetodo{validateEmail}{}{Promise}{ritorna una promise che risolve senza errori solamente se la nuova password è valida}
\classeMetodo{validateUsername}{username}{Promise}{ritorna una promise che risolve senza errori solamente se l'username è valido e non ne esiste uno di uguale già registrato}
\begin{classeMetodoArgomenti}
\classeMetodoArgomento{username}{string}{l'username da validare}
\end{classeMetodoArgomenti}
\end{classeMetodi}
...
\begin{classeMetodi}
\classeMetodo{generate}{}{string}{genera una stringa casuale univoca}
\end{classeMetodi}
...
\begin{classeMetodi}
\classeMetodo{getToken}{}{string}{metodo che restituisce il token dell'utente che effettuata la richiesta dal client}
\classeMetodo{getUserID}{token}{string}{metodo che a partire dal token di autenticazione ritorna l'id dell'utente che effettua la richiesta}
\begin{classeMetodoArgomenti}
\classeMetodoArgomento{token}{string}{token di autenticazione dell'utente}
\end{classeMetodoArgomenti}
\end{classeMetodi}
