...
\post{l'utente è autenticato}\\\\ 
\scen{\UC{UC1.1.1.1} l'utente inserisce l'email,
\UC{UC1.1.1.2} l'utente inserisce la password,
\UC{UC1.1.1.3} l'utente conferma di voler eseguire il login.}\\\\ 
\att{Utente}
...
\casoduso{UC1.1.1.3.1}{Visualizza info dati login errati} 
...
\casoduso{UC1.1.1.3.2}{Resetta la password} 
...
\scen{se l'utente non è autenticato:
 - \UC{UC1.5.4} l'utente viene informato della possibilità di salvare i percorsi effettuati se si autentica,
altrimenti, se l'utente non ha percorsi registrati:
 - \UC{UC1.5.3} l'utente viene invitato a compiere un percorso per poi visualizzarne il dettaglio in questa pagina
altrimenti:
 - \UC{UC1.5.1} l'utente visualizza l'elenco dei percorsi svolti,
 - \UC{UC1.5.2} l'utente può visualizzare il dettaglio di un percorso salvato.}\\\\ 
\att{Utente, Utente autenticato}
...
\begin{figure}[H] 
\centering 
\includegraphics[scale=0.4]{img/UC_1_6_png} 
